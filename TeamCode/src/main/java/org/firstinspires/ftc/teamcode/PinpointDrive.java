...
public class PinpointDrive extends MecanumDrive {
    ...
    public PinpointDrive(HardwareMap hardwareMap, Pose2d pose) {
        ...
        pinpoint = hardwareMap.get(GoBildaPinpointDriverRR.class, PinpointPARAMS.pinpointDeviceName);

        if (PinpointPARAMS.usePinpointIMUForTuning) {
            lazyImu = new LazyImu(hardwareMap, PinpointPARAMS.pinpointDeviceName, new RevHubOrientationOnRobot(zyxOrientation(0, 0, 0)));
        }

        // RR localizer note: don't love this conversion (change driver?)
        pinpoint.setOffsets(DistanceUnit.MM.fromInches(PinpointPARAMS.xOffset), DistanceUnit.MM.fromInches(PinpointPARAMS.yOffset));

        pinpoint.setEncoderResolution(PinpointPARAMS.encoderResolution);
